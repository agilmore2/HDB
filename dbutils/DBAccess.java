...
/**  Public class DBAccess is used to provide ORACLE database connectivity
     close the connection to the database and to provide query methods to 
     query the currect database.  The system property db.property must
     be set inorder for the database connection to function properly.
...
public class DBAccess
{
    ...
    // conn instance variable is used to store the current connection object to the database
...
/** Public method getConnection establishes the database connection if the 
    ...
    connectivity requires that the system property "db.property" must be
    set before calling this method. The db.property property must be a pointer 
    to an actual property file that contains the following database properties:

    1. Database Driver:   (eg. DRIVER=oracle.jdbc.driver.OracleDriver)
    2. Database URL:     (eg.  URL=jdbc:oracle:thin:@stork.central:1521:CFUATAM)
    3. Schema Identifier: (eg. USERNAME=cust_docs)
    4. Schema Password:   (eg. PASSWORD=cust_docs)
...
*/  
    public Connection getConnection(DataObject do1)
    {
        ...
        try 
        {
          ...
          if (((String)do1.get("DEBUG")).equals("Y")) log.debug(url + " connection successful");
        ...
        finally // in all case return the connection
        {
          return conn;
        }
    }
...
/** 
    ...
    Public method perform DML
    takes the supplied input variable String
    and executes this DML   against the database.  Tthis method will allow preformed DML statemants
    That do inserts, updates deletes and DDL statements can be used here
...
/** 
    ...
    @version 1.0
    @param query the Actual sql formatted query that is to be performed.
    @param do1   The dataobject the query results will be placed into.  
    Date:   09-April-2001
*/ 
       ...
       try
       {
          /*  performQuery 
            ---------------------------
             ...
             For each row:
              create an arraylist of string values for all columns in the row
              add that row to the records arraylist 
              increment row count
             If only one row add each column and its (String) value to the D.O.
          ...
          */
            ...
            if (rowCount == 0)
            {
              ...
              for (int i = 0; i < cols; i++)
              { 
                // get the column key from metadata object and the empty String temp
            ...
            if (rowCount == 1)
            {
              ...
              { 
                ...
                // get the column key from metadata object and the value from
                // the arraylist for that columns entry and add these entries
                // to the dataObject hastable
                do1.put((String)columns.get(i),(String) ((ArrayList)records.get(0)).get(i));
              }
            }
            else if (rowCount > 1) // there were multiple rows returned	
            {
              ...
              { 
                ...
                for (int j = 0; j < rowCount; j++) // go to every row, get column value
                {
                  // add column data to the column arrayList
                  columnData.add(((ArrayList)records.get(j)).get(i));
                }
                // done with this column, add this arrayList to the dataObject
...
*/
    ...
    /** Public method performCall performs the database procedure call
        ...
        @author Mark A. Bogner
        @param _field  The  merge field that is being requested data for
        @param do1  the Dataobject that the mereg field and its associated 
    ...
    */
    ...
    /** Public method initialCall performs an initial call to the database
        procedure which is required to set up the initial input parameters.
        these parameters are used to initislize the database connection so
        that the procedure call knows what contract, database  language etc..
        the document is related to.

        @author Mark A. Bogner
        @param  do1 The dataobject that will contain the data items this 
         procedure needs
