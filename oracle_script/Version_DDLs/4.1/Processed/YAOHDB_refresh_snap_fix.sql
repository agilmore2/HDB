...
drop trigger YAOHDBA.HDB_DATATYPE_ARCHIVE_DELETE;
drop trigger YAOHDBA.HDB_DATATYPE_ARCHIVE_UPDATE;
drop trigger YAOHDBA.HDB_EXT_DATA_CODE_ARCH_DEL;
drop trigger YAOHDBA.HDB_EXT_DATA_CODE_ARCH_UPD;
drop trigger YAOHDBA.HDB_EXT_DATA_SOURCE_ARCH_DEL;
drop trigger YAOHDBA.HDB_EXT_DATA_SOURCE_ARCH_UPD;
drop trigger YAOHDBA.HDB_EXT_SITE_CODE_ARCH_DEL;
drop trigger YAOHDBA.HDB_EXT_SITE_CODE_ARCH_UPD;
drop trigger YAOHDBA.HDB_SITE_ARCHIVE_DELETE;
drop trigger YAOHDBA.HDB_SITE_ARCHIVE_UPDATE;
drop trigger YAOHDBA.HDB_SITE_DATATYPE_ARCH_DELETE;
drop trigger YAOHDBA.HDB_SITE_DATATYPE_ARCH_UPDATE;

drop table YAOHDBA.HDB_DATATYPE_ARCHIVE purge;
drop table YAOHDBA.HDB_SITE_DATATYPE_ARCHIVE purge;
drop table YAOHDBA.HDB_SITE_ARCHIVE purge;

CREATE SNAPSHOT YAOHDBA.HDB_DATATYPE_ARCHIVE PCTFREE 10 PCTUSED 40 TABLESPACE HDB_DATA storage (initial 50k next 50k pctincrease 0) refresh complete with rowid AS SELECT * FROM  UCHDBA.HDB_DATATYPE_ARCHIVE@UCHDB2;
CREATE SNAPSHOT YAOHDBA.HDB_SITE_DATATYPE_ARCHIVE PCTFREE 10 PCTUSED 40 TABLESPACE HDB_DATA storage (initial 50k next 50k pctincrease 0) refresh complete with rowid AS SELECT * FROM  UCHDBA.HDB_SITE_DATATYPE_ARCHIVE@UCHDB2;
CREATE SNAPSHOT YAOHDBA.HDB_SITE_ARCHIVE PCTFREE 10 PCTUSED 40 TABLESPACE HDB_DATA storage (initial 50k next 50k pctincrease 0) refresh complete with rowid AS SELECT * FROM  UCHDBA.HDB_SITE_ARCHIVE@UCHDB2;


create or replace PROCEDURE     YAOHDBA.refresh_HDB_snap  (which_table IN varchar2)
   IS
