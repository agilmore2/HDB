...
comment on table REF_SPATIAL_RELATION is 'Defines one site's relation to another, spatially. Uses HDB_ATTR entries to define the relationships between sites, and can represent a fractional value.';
comment on table REF_STR is 'Holds information on stream gages.Since the table only allows a limited amount of information – area, gage type and owner – it would be better to define attributes common to gages (in hdb_attr) and then populate the ref_attr table. ';
...
comment on table CP_COMP_PROC_LOCK is 'Heartbeat table for computation processes.';
...
comment on table R_BASE_ARCHIVE is 'Archive table of R_BASE. Holds the data that previously resided in r_base but was modified or deleted. This archive table can be used to re-generate the data set that was active at any point in time.';
