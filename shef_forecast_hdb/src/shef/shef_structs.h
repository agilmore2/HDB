...
/*    
       ...
       DEC 21 97 DGB
                ...
                This flag is set on if an explicit century is specified 
                in the positional data.
       JAN  5 98 DGB
                Add database in tempfiles for compatibility with
                HP posting.
       JAN  8 98 DGB
                Add century flag.   Set to on ( i.e. 1) if year
                is not explicitly given in the positional data
                and a year has to be assumed. This can explicitly set the
                system time to a century.  Used for testing.
       JAN 17 98 DGB
                Add yearflag flag.  Set to on ( i.e. 1) if year
                is explicitly given in the positional data
                and a year has to be assumed. 
       SEP 17 98 DGB
                Add dotend, and dotbegin.
       NOV 14 98 DGB
                Add temp1 and temp2 to tempfiles structure for
                dot e preprocessing.
...
int dotend, dotbegin, econtinuation, efound;                 /* dgb:09/17/98 */
int DOTEPRE, INFORMIX_ERR, SENSORS, PARTIAL_ERROR, DEBUG, VERBOSE;
int num_records;
int FCFG;
int DATABASE_NAME;
int CHECKTABLES;
...
int STATS;
int tid;
int TURNONR;	                                            /* dgb:01/28/00 */

FILE *fp_postlog, *sqlerrors, *sensors, *testfile;
