...
my $decdir = "$ENV{HDB_ENV}/ratings/cdwr/src";
...
#the ugly globals...
my $agen_abbrev      = 'CDWR';
my $data_source      = 'Colorado Division of Water Resources (CODWR)';
...
b. primary_data_code like 'DISCHRG%'
order by siteno",
  'siteno'
  );
...
####Load $cs $csd $wm to HDB this is the current shift, date of shift and web message   
   #print "CODWR SITE: $site CURRENT SHIFT VALUE: $cs RATING TABLE: $rt SHIFT CURVE: $sc WEB MESSAGE: $wm\n";
    if ($cs eq 'SC'){ 
    print STDERR "PROCESSING SITE WITH SHIFT CURVE:  $site\n";
